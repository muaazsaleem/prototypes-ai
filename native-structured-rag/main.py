...
def embed_all(chunks: List[str], label: str) -> List[List[float]]:
    ...
    for i, chunk in enumerate(chunks):
        console.print(f"  [dim][{label}][/dim] embedding chunk {i+1}/{len(chunks)} ...", end="\r")
...
def main():
    ...
    naive_avg  = sum(len(c) for c in naive_c)  // len(naive_c)
    struct_avg = sum(len(c) for c in struct_c) // len(struct_c)

    console.print(Rule("[bold]Document Stats[/bold]", style="white"))
    console.print(f"  Total chars          : [bold]{len(doc):,}[/bold]")
    console.print(f"  Naive chunks         : [cyan]{len(naive_c):>3}[/cyan]   (avg {naive_avg:>5} chars)")
    console.print(f"  Structured chunks    : [magenta]{len(struct_c):>3}[/magenta]   (avg {struct_avg:>5} chars)")
    console.print()
    ...
    for idx, question in enumerate(QUESTIONS):
        console.print(f"[bold cyan]-- Q{idx+1} ----------------------------------------------------[/bold cyan]")
        ...
        ans_a     = answer(question, [c for c, _ in top_a])
        ...
        ans_b         = answer(question, [c for c, _ in top_b])
        ...
        console.print(f"  [cyan]Naive[/cyan]      (k=3) q:{quality_a}/5  sim: {', '.join(sim_a)}")
        console.print(f"    [dim]A: {ans_a}[/dim]")
        console.print(f"  [magenta]Structured[/magenta] (k=2) q:{quality_b}/5  rel: {', '.join(rel_b)}")
        console.print(f"    [dim]A: {ans_b}[/dim]")
    ...
    summary_table.add_row("Avg Quality", f"{avg_naive:.1f} / 5", f"{avg_struct:.1f} / 5")
