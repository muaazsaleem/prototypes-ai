...
def _system_prompt(output_path: str) -> str:
    return f"""You are an expert software engineer that implements algorithms and systems from \
research papers.

## Your task
Read the paper provided in the first conversation message and produce working, well-commented Python code that implements the \
...
- `bash`             -- execute a bash command (e.g. to run unit/integration tests, check dependencies/environment, or run other shell commands as required)
...
- You can use the `bash` tool if required, for example to run test files, inspect file systems, or run terminal commands to verify the workspace or code execution.
"""
...
def run(state: AgentState) -> str:
    ...
    system_instruction = _system_prompt(state.output_path)

    # Initial user request with the paper text included
    user_content = types.Content(
        role="user",
        parts=[types.Part(text=(
            f"Here is the text of the paper:\n\n---\n{state.paper_text[:60_000]}\n---\n\n"
