...
class TestBash:
    def test_runs_valid_bash_command(self):
        result, done, code_out = dispatch_tool("bash", {"command": "echo 'hello bash'"}, "")
        assert done is False
        assert code_out is None
        assert "hello bash" in result
        assert "exit_code=0" in result

    def test_captures_bash_stderr(self):
        result, _, _ = dispatch_tool("bash", {"command": "echo 'error message' >&2; exit 42"}, "")
        assert "error message" in result
        assert "exit_code=42" in result

    def test_handles_empty_command(self):
        result, done, _ = dispatch_tool("bash", {"command": "  "}, "")
        assert "ERROR" in result
        assert done is False
